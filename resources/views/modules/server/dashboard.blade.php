<x-layouts.app title="Dashboard: {{ $server->name }}">
    <section class="w-full max-w-6xl">
        ...
        {{-- Terminal Tab --}}
        <div id="tab-terminal" class="tab-content mt-6 hidden" data-tab="terminal">
            <div class="rounded-2xl bg-white p-4 shadow-[inset_0_0_0_1px_rgba(26,26,0,0.16)] dark:bg-[#161615] dark:shadow-[inset_0_0_0_1px_#fffaed2d] sm:p-6">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    <div>
                        <p class="text-sm text-[#f53003] dark:text-[#FF4433]">SSH-Terminal</p>
                        <p class="mt-1 text-xs text-[#706f6c] dark:text-[#A1A09A]">Interaktive PTY-Session als {{ $server->username }}<span>@</span>{{ $server->host }}</p>
                    </div>
                    <div class="flex items-center gap-2">
                        ...
                        <button type="button" id="terminal-reconnect" onclick="reconnectTerminal()" class="rounded-lg border border-[#19140035] px-3 py-1.5 text-xs hover:border-[#1915014a] dark:border-[#3E3E3A] dark:hover:border-[#62605b]">
                            Neu verbinden
                        </button>
                    </div>
                </div>

                <div id="terminal-container" class="mt-4 h-[460px] overflow-hidden rounded-xl border border-[#19140020] bg-[#0b0f14] p-2 dark:border-[#3E3E3A]"></div>
                ...
                fetch('{{ route('server.terminal.session', $server) }}', {
                    ...
                    .then(data => {
                        ...
                        socket.addEventListener('close', () => {
                            if (document.getElementById('tab-terminal')?.classList.contains('hidden') === false) {
                                setTerminalStatus('Getrennt', 'idle');
                            }
                        });
    ...
    document.querySelectorAll('.tab-btn').forEach(btn => {
        btn.addEventListener('click', function () {
            ...
            if (this.dataset.tab === 'services') {
                ...
                loadGithubTab();
            } else if (this.dataset.tab === 'terminal') {
                loadTerminalTab();
            }
        });
    });
