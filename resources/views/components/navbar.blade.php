@php
    ...
    $allServerRoutes = array_merge(
        array_column($systemLinks, 'active'),
        array_column($diensteLinks, 'active'),
        ['server.firewall.*'],
        array_column($datenLinks, 'active'),
    );
@endphp
        ...
        <div class="hidden items-center gap-1 text-sm lg:flex">
            ...
            @if ($currentServer)
                ...
                <div class="group relative">
                    <button type="button" class="{{ request()->routeIs('server.system', 'server.terminal', 'server.monitoring.*', 'server.logs.*') ? $activeLinkClass : $linkClass }} inline-flex items-center gap-1">
                        System
                ...
                {{-- Dienste --}}
                <div class="group relative">
                    <button type="button" class="{{ request()->routeIs('server.services.*', 'server.docker.*', 'server.cronjobs.*', 'server.apache.*', 'server.nginx.*', 'server.mysql.*') ? $activeLinkClass : $linkClass }} inline-flex items-center gap-1">
                        Dienste
                        <svg class="size-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                    </button>
                    <div class="absolute left-0 top-full z-40 hidden w-48 rounded-xl border border-[#19140020] bg-white p-2 text-sm shadow-lg group-hover:block group-focus-within:block dark:border-[#3E3E3A] dark:bg-[#161615]">
                        @foreach ($diensteLinks as $link)
                            <a href="{{ route($link['route'], $currentServer) }}" class="{{ $dropdownLinkClass }} {{ request()->routeIs($link['active']) ? 'font-medium' : '' }}">{{ $link['label'] }}</a>
                        @endforeach
                    </div>
                </div>
                ...
                {{-- Firewall --}}
                <a href="{{ route('server.firewall.index', $currentServer) }}" class="{{ request()->routeIs('server.firewall.*') ? $activeLinkClass : $linkClass }}">Firewall</a>

                {{-- Daten --}}
                <div class="group relative">
                    <button type="button" class="{{ request()->routeIs('server.files.*', 'server.backups.*', 'server.github.*') ? $activeLinkClass : $linkClass }} inline-flex items-center gap-1">
                        Daten
                        <svg class="size-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                    </button>
                    <div class="absolute left-0 top-full z-40 hidden w-48 rounded-xl border border-[#19140020] bg-white p-2 text-sm shadow-lg group-hover:block group-focus-within:block dark:border-[#3E3E3A] dark:bg-[#161615]">
                        @foreach ($datenLinks as $link)
                            <a href="{{ route($link['route'], $currentServer) }}" class="{{ $dropdownLinkClass }} {{ request()->routeIs($link['active']) ? 'font-medium' : '' }}">{{ $link['label'] }}</a>
                        @endforeach
                    </div>
                </div>
            @else
                <span class="{{ $disabledLinkClass }}" aria-disabled="true" title="Bitte zuerst Server auswählen">System</span>
                <span class="{{ $disabledLinkClass }}" aria-disabled="true" title="Bitte zuerst Server auswählen">Dienste</span>
                <span class="{{ $disabledLinkClass }}" aria-disabled="true" title="Bitte zuerst Server auswählen">Firewall</span>
                <span class="{{ $disabledLinkClass }}" aria-disabled="true" title="Bitte zuerst Server auswählen">Daten</span>
            @endif
        ...
        <div id="navbar-mobile-menu" class="absolute left-4 right-4 top-full z-40 mt-2 hidden rounded-xl border border-[#19140020] bg-white p-3 text-sm shadow-lg dark:border-[#3E3E3A] dark:bg-[#161615] lg:hidden" data-navbar-menu="mobile">
            <div class="grid gap-1">
                ...
                @if ($currentServer)
                    {{-- Mobile: System --}}
                    <div class="relative">
                        <button type="button" onclick="this.nextElementSibling.classList.toggle('hidden')" class="{{ request()->routeIs('server.system', 'server.terminal', 'server.monitoring.*', 'server.logs.*') ? $activeLinkClass : $linkClass }} inline-flex w-full items-center justify-between gap-1">
                            System
                            <svg class="size-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                        </button>
                    ...
                    {{-- Mobile: Dienste --}}
                    <div class="relative">
                        <button type="button" onclick="this.nextElementSibling.classList.toggle('hidden')" class="{{ request()->routeIs('server.services.*', 'server.docker.*', 'server.cronjobs.*', 'server.apache.*', 'server.nginx.*', 'server.mysql.*') ? $activeLinkClass : $linkClass }} inline-flex w-full items-center justify-between gap-1">
                            Dienste
                            <svg class="size-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                        </button>
                        <div class="ml-3 mt-1 hidden space-y-1 border-l border-[#19140020] pl-3 dark:border-[#3E3E3A]">
                    ...
                    {{-- Mobile: Daten --}}
                    <div class="relative">
                        <button type="button" onclick="this.nextElementSibling.classList.toggle('hidden')" class="{{ request()->routeIs('server.files.*', 'server.backups.*', 'server.github.*') ? $activeLinkClass : $linkClass }} inline-flex w-full items-center justify-between gap-1">
                            Daten
                            <svg class="size-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                        </button>
                        <div class="ml-3 mt-1 hidden space-y-1 border-l border-[#19140020] pl-3 dark:border-[#3E3E3A]">
